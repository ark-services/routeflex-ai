...
export default function FieldCard({
  ...
  const renderFieldPreview = () => {
    ...
    switch (field.type) {
      case "text":
        return (
          <input
            ...
            className="w-full px-3 py-2 border border-rf-border rounded-lg bg-rf-surface-page text-sm text-rf-text-muted cursor-not-allowed"
            disabled
          />
        );
      case "textarea":
        return (
          <textarea
            placeholder={placeholderText || "Type your answer here..."}
            className="w-full px-3 py-2 border border-rf-border rounded-lg bg-rf-surface-page text-sm text-rf-text-muted cursor-not-allowed resize-none"
            rows={3}
            disabled
          />
        );
      ...
      case "file":
        return (
          <div className="w-full border-2 border-dashed border-rf-border rounded-lg bg-rf-surface-page p-6 text-center">
            <div className="text-rf-text-muted mb-2">
              <svg className="mx-auto h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
            <p className="text-sm text-rf-text-muted">Drag and drop or click to upload</p>
          </div>
        );
      case "radio":
        ...
        return (
          <div className="space-y-2">
            {radioOptions.map((option: string, idx: number) => (
              <label key={idx} className="flex items-center gap-2 text-sm text-rf-text-secondary">
        ...
        return (
          <div className="flex items-center gap-2.5">
            <input
              ...
              className="h-4 w-4 rounded border-rf-border cursor-not-allowed"
            />
            <span className="text-sm text-rf-text-muted">{field.label}</span>
          </div>
        );
      case "select":
        ...
        return (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border border-rf-border rounded-lg bg-rf-surface-page text-sm text-rf-text-muted cursor-not-allowed"
              disabled
            />
          </div>
        );
    ...
    <div
      ...
      className={`
        ...
        ${isSelected
          ...
          : "border-rf-border hover:border-rf-border shadow-sm hover:shadow-md"
        }
      `}
    >
      ...
      <div className="absolute top-4 right-4 flex flex-col items-end gap-2">
        <span className="text-xs text-rf-text-muted uppercase tracking-wider font-medium">
          {field.type}
        </span>
        {field.required && (
      ...
      <div className="mb-2 pr-20">
        {isEditingLabel ? (
          <input
            ...
            className="w-full text-lg font-semibold text-rf-ink-900 border-b-2 border-rf-blue focus:outline-none px-1 -mx-1"
          ...
          <h3
            ...
            className="text-lg font-semibold text-rf-ink-900 hover:text-rf-blue cursor-text px-1 -mx-1 rounded hover:bg-rf-surface-page"
          >
            {field.label}
          </h3>
        )}
      ...
      <div className="mb-4">
        {isEditingDescription ? (
          <input
            ...
            className="w-full text-sm text-rf-text-secondary border-b border-rf-blue focus:outline-none px-1 -mx-1"
            onClick={(e) => e.stopPropagation()}
          />
        ) : (
          <p
            ...
            className="text-sm text-rf-text-secondary hover:text-rf-ink-900 cursor-text px-1 -mx-1 rounded hover:bg-rf-surface-page min-h-[20px]"
          >
      ...
      {field.settings?.imageUrl && (
        <div className="mb-4 rounded-lg overflow-hidden border border-rf-border">
      ...
      {isSelected && (
        <div className="flex items-center gap-2 pt-4 border-t border-rf-border">
          <button
            onClick={(e) => {
              e.stopPropagation();
              onOpenSettings();
            }}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-rf-ink-700 hover:bg-rf-ink-100 rounded-lg transition-colors"
          >
