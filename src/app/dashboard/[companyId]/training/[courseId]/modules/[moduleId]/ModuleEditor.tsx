...
export function ModuleEditor({ companyId, courseId, module: mod, questions: initialQuestions }: Props) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaved(false);
    try {
      await updateModule(companyId, courseId, mod.id, { title, content });
      setSaved(true);
  ...
  async function handleAddQuestion() {
    ...
    try {
      const id = await createQuestion(companyId, courseId, mod.id, newQ);
      setQuestions((prev) => [
        ...prev,
  ...
  async function handleGenerateQuestions() {
    ...
    try {
      ...
      for (const q of questions) {
        await deleteQuestion(companyId, courseId, mod.id, q.id);
      }
...
function QuestionEditor({
  ...
  async function handleSave() {
    ...
    setSaved(false);
    try {
      await updateQuestion(companyId, courseId, moduleId, localQ.id, {
