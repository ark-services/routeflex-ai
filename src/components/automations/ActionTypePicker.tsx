  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full px-3 py-2.5 border-2 border-dashed border-emerald-300/50 rounded-rf-lg text-rf-text-muted hover:border-emerald-400 hover:text-emerald-600 hover:bg-emerald-50/40 transition-all flex items-center justify-center gap-1.5 text-sm font-medium"
      >
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute z-20 left-0 right-0 mt-2 bg-rf-surface-card border border-rf-border rounded-rf-lg shadow-rf-xl max-h-80 overflow-hidden flex flex-col">
            {/* Search */}
            <div className="p-2.5 border-b border-rf-border">
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-rf-ink-300" />
                <input
                  type="text"
                  placeholder="Search actions..."
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-8 pr-3 py-1.5 text-sm border border-rf-ink-100 rounded-rf-md focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-1 placeholder:text-rf-ink-300"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="overflow-y-auto">
              {filtered.map((cat) => (
                <div key={cat.label}>
                  <div className="px-3 py-1.5 text-[11px] font-bold text-rf-ink-300 uppercase tracking-widest bg-rf-surface-page/80">
                    {cat.label}
                  </div>
                  {cat.actions.map((action) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full px-3.5 py-2 text-left hover:bg-emerald-50 transition-colors flex items-center gap-2.5 text-sm"
                      >
                        <Icon className="w-4 h-4 text-rf-ink-300 flex-shrink-0" />
